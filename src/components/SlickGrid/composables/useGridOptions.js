...
const BASE_OPTIONS = {
   ...
   * ----- 헤더 메뉴/버튼 정리 -----
   *
   * 라이브러리 기본값은 컬럼 픽커(우클릭 팝업) / 헤더 메뉴(컬럼마다 ∨ 버튼) / 그리드 메뉴(우측 상단 ☰)
   * 가 전부 켜져 있어 헤더가 번잡하다. 다음과 같이 정리한다.
   *
...
const featuresToOptions = (features) => {
  ...
  // ----- 그리드 메뉴 -----
  // 컬럼 픽커(우클릭 팝업)와 헤더 메뉴(∨)는 BASE_OPTIONS에서 껐으므로 여기서 다시 켜지 않는다.
  // 그리드 메뉴는 ☰ 버튼 없이 헤더 우클릭으로 열린다.
