...
정렬 = 헤더 좌클릭 3단계
  오름차순 → 내림차순 → 해제(원래 순서로 복원, 정렬 화살표도 사라짐)
  한 번에 한 컬럼만 정렬됩니다. 다른 컬럼을 누르면 이전 정렬은 풀립니다.
  해제 시 복원 기준은 행 고유 키(id-field) 오름차순입니다. 받은 순서가 키 순서와 다르면
    :options="{ defaultColumnSortFieldId: 'sortSeq' }" 로 기준 필드를 지정하세요.
  서버 모드(fetch-data)에서는 sorters 가 빈 배열로 재조회되어 서버 순서가 그대로 옵니다.
  여러 컬럼 누적 정렬이 필요하면 :options="{ multiColumnSort: true }"
