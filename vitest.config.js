...
환경(happy-dom) 주의:
  SlickGrid / DevExtreme DataGrid 본체는 실제 브라우저의 레이아웃 계산(offsetWidth/스크롤)에 의존하므로
  happy-dom 에서는 정상 렌더되지 않는다.
  (DevExtreme 페이저는 자식 요소의 computed width 를 읽어 적응형 모드를 정하기 때문에 특히 그렇다)
  그래서 SlickGrid.vue / DevExtremeGrid.vue 자체를 마운트하는 테스트는 두지 않고,
  - 순수 로직(composables)
  - 그리드 라이브러리에 의존하지 않는 Vue 컴포넌트(SlickGridPagination, CodeCard)
